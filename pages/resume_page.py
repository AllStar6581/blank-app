...
_VK_VIDEO_RE = re.compile(r"https?://(?:m\.)?(?:vkvideo\.ru|vk\.com)/video(-?\d+)_(\d+)")
...
def _video_embed_html(link: str) -> str | None:
    """Return raw embed HTML for a single video (VK iframe or YouTube), or None."""
    vk_match = _VK_VIDEO_RE.match(link)
    if vk_match:
        oid, vid = vk_match.groups()
        embed_url = f"https://vkvideo.ru/video_ext.php?oid={oid}&id={vid}&hd=2"
        return (
    ...
    if yt_match:
        ...
        if _EXPERIMENTAL_YT_FACADE:
            return (
                f'<div class="yt-facade" onclick="'
                f"this.outerHTML='<iframe src=&quot;https://www.youtube.com/embed/{vid_id}"
                f"?autoplay=1&quot; style=&quot;width:100%;height:100%&quot; frameborder=&quot;0&quot;"
                f" allow=&quot;autoplay;encrypted-media&quot; allowfullscreen></iframe>'\">"
                f'<div class="yt-play"></div></div>'
                f"<style>.yt-facade{{width:100%;height:100%;cursor:pointer;"
                f"background:#000 url('https://i.ytimg.com/vi/{vid_id}/hqdefault.jpg')"
                f" center/cover no-repeat;display:flex;align-items:center;"
                f"justify-content:center}}"
                f".yt-play{{width:68px;height:48px;background:rgba(255,0,0,.8);"
                f"border-radius:14px;display:flex;align-items:center;justify-content:center;"
                f"transition:background .2s}}"
                f".yt-facade:hover .yt-play{{background:#f00}}"
                f".yt-play::after{{content:'';border-style:solid;"
                f"border-width:11px 0 11px 19px;"
                f"border-color:transparent transparent transparent #fff;"
                f"margin-left:4px}}</style>"
            )
        return (
...
def _video_carousel_html(links: list[str]) -> str:
    ...
    for i, link in enumerate(links):
        embed = _video_embed_html(link)
        if embed is None:
            continue
        active = "active" if i == 0 else ""
        items_html += f'<div class="carousel-item" id="slide-{i}">{embed}</div>'
        dot_html += f'<span class="dot {active}" data-idx="{i}"></span>'
...
.carousel {{
    ...
    width:100%; height:calc(100% - 30px); scroll-behavior:smooth;
...
.dot.active {{ background:#f00; }}
</style></head><body>
<div class="carousel" id="carousel">{items_html}</div>
<div class="dots" id="dots">{dot_html}</div>
<script>
(function() {{
    ...
    var observer = new IntersectionObserver(function(entries) {{
        entries.forEach(function(e) {{
            if (e.isIntersecting) {{
                var idx = Array.prototype.indexOf.call(carousel.children, e.target);
...
def _render_video(link: str) -> None:
    """Render a video: VK Video via lazy iframe, YouTube via facade or st.video."""
    match = _VK_VIDEO_RE.match(link)
    if match:
